...
class PMLTool
  ...
  def print_flowfacts(io = $stdout)
    print_by_origin(pml.flowfacts.list, "flowfacts", io) do |ffs|
      ffs.group_by do |ff|
        ff.classification_group(@pml)
      end.each do |klass,ffs2|
        puts "--- #{klass} ---"
        ffs2.each do |ff|
          puts ff
        end
  ...
  def print_valuefacts(io = $stdout)
    print_by_origin(pml.valuefacts.list, "valuefacts", io) do |vfs|
      vfs.each { |vf| puts vf }
    end
  end
  ...
  def print_by_origin(set, setname, io)
    set.group_by do |info|
      info.origin
    end.each do |origin,infos|
  ...
  def self.run(pml,options)
    ...
    if options.stats
      ...
      pml.flowfacts.stats(pml).each do |level,by_group|
        by_group.each do |group, by_class|
          next if group == :cnt
